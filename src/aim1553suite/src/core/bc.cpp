#include "bc.hpp"
#include "FrameComponent.hpp" // DEĞİŞİKLİK: FrameComponent'in tam yolunu belirtmek daha robust bir yaklaşım.
#include <cstring>
#include <stdexcept>
#include <thread>
#include <chrono>
...
AiReturn BusController::initialize(int deviceId, int streamId) {
    ...
    // DEĞİŞİKLİK: ApiInit() çağrısı buradan kaldırıldı.
    // Bu işlem artık merkezi olarak App::OnInit içinde yapılıyor.
    // if (ApiInit() < 1) {
...
void BusController::shutdown() {
    ...
    if (m_boardHandle != 0) {
        ...
        ApiClose(m_boardHandle); // Bu sınıfa ait handle'ı kapatmak doğru.
        m_boardHandle = 0;
    }
    
    // DEĞİŞİKLİK: ApiExit() çağrısı buradan kaldırıldı.
    // Bu işlem artık merkezi olarak App::OnExit içinde yapılıyor.
    // ApiExit(); 
