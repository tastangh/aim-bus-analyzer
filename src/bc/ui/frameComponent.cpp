...
FrameComponent::FrameComponent(wxWindow *parent, const FrameConfig &config)
    ...
    // DÜZELTME: Ana pencereye ulaşmak için hiyerarşide bir seviye yukarı çıkmak yeterli.
    // parent = m_scrolledWindow, parent->GetParent() = BusControllerFrame
    ...
    auto *editFrameButton = new wxButton(this, wxID_ANY, "Edit", wxDefaultPosition, wxSize(120, -1));
...
void FrameComponent::updateValues(const FrameConfig &config) {
    ...
    case BcMode::MODE_CODE_WITH_DATA: ss << "BC -> RT " << m_config.rt << " | MC " << m_config.sa << (m_config.mode == BcMode::MODE_CODE_WITH_DATA ? " (w/ data)" : ""); break;
    }
...
void FrameComponent::sendFrame() {
    ...
    if (!m_mainWindow) {
        std::cerr << "[UI] HATA: Ana pencere bulunamadı!" << std::endl;
        return;
    }
    ...
    AiReturn status = bc.sendAcyclicFrame(m_config, received_data);
...
void FrameComponent::onRemove(wxCommandEvent &) { 
    std::cout << "[UI] '" << m_config.label << "' çerçevesi siliniyor." << std::endl;
    if (m_mainWindow) {
