 ...
 // Makro ile kullanımı kolaylaştırma
 #define CHECK_API(ret, func_name) check_api_return(ret, func_name, __FILE__, __LINE__)
 
 // =================================================================================
 // MilUtil YARDIMCI FONKSİYONLARI (Namespace olarak C++'a uyarlandı)
 // =================================================================================
 namespace MilUtil 
 {
     // Orijinal mil_util.c'deki static global değişkenlerin C++ karşılığı
     ...
     void SetData(AiUInt32 modHandle, int biuId, AiUInt16 headerId, AiUInt16 index, const AiUInt16* data, AiUInt16 wordCount) {
         // HATA GİDERİLDİ: Çıkış parametreleri için geçerli değişkenler oluşturuldu.
         AiUInt16 outIndex;
         AiUInt32 outAddr;
     ...
     void CreateTransfer(AiUInt32 modHandle, int biuId, AiUInt16 transferId, AiUInt8 type, AiUInt16 headerId, AiUInt8 txRt, AiUInt8 txSa, AiUInt8 rxRt, AiUInt8 rxSa, AiUInt8 wcnt) {
         ...
         bc_xfer.rcv_sa = rxSa;
         bc_xfer.wcnt = wcnt;
         // HATA GİDERİLDİ: Çıkış parametresi için geçerli bir değişken adresi sağlandı.
         AiUInt32 desc_addr;
 ...
 // --- Orijinal C Örneğindeki Fonksiyonların C++ Karşılıkları ---
 ...
 void RunSimulation(AiUInt32 board_handle, int biu_id) {
     ...
     do {
         ...
         // Durum kontrolü MilUtil::IsBcActive içinde yapılıyor
